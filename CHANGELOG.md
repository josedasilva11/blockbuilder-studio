...
- **Chamfered box (primitive parameter).** Box now has a `Chamfer` parameter alongside Width / Depth / Height. Set it to `0` for the classic sharp box, or to anything up to ~half the smallest dimension for a 45° bevel along every edge. The geometry is regenerated analytically (24 unique vertices, 6 main faces, 12 edge chamfer quads, 8 corner triangles, 44 triangles total) — clean watertight mesh that survives CSG operations and STL export. Other primitives don't have chamfer yet; this only applies to Box for now.
